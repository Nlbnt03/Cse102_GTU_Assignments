...
int main() {
    ...
    while (scanf("%c", &karakter) != EOF) {
        if (karakter == '/' && scanf("%c", &karakter) && karakter == '*') { // Comment marker detected
            ...
            // Onlar basamağını alıp şifreleme
            char onlar = (comment_count / 10) + '0';
            int tmp_index = -1;
            for (int i = 0; i < 61; i++) {
                if (onlar == alphabet[i]) {
                    tmp_index = i;
            ...
            // Encrypt and write character if found
            if (tmp_index != -1) {
                int new_index = (tmp_index + key) % 61;
                printf("%c", alphabet[new_index]);
            } else {
                // Write non-alphabetical characters directly
                printf("%c", onlar);
            }
            // Birler basamağını alıp şifreleme
            char birler = (comment_count % 10) + '0';
            int tmp_index2 = -1;
            for (int i = 0; i < 61; i++) {
                if (onlar == alphabet[i]) {
                    tmp_index2 = i;
            ...
            // Encrypt and write character if found
            if (tmp_index2 != -1) {
                int new_index = (tmp_index2 + key) % 61;
                printf("%c", alphabet[new_index]);
            } else {
                // Write non-alphabetical characters directly
                printf("%c", birler);
            ...
            // Convert uppercase to lowercase for case-insensitive encryption
